...
data class WEATHERV (val response : RESPONSE)
data class RESPONSEV(val header : HEADER, val body : BODY)
data class HEADERV(val resultCode : Int, val resultMsg : String)
data class BODYV (val dataType : String, val items : ITEMS, val totalCount : Int)
data class ITEMSV(val item : List<ITEM>)
// category : 자료 구분 코드, fcstDate : 예측 날짜, fcstTime : 예측 시간, fcstValue : 예보 값
data class ITEMV(val category : String, val fcstDate : String, val fcstTime : String, val fcstValue : String)
